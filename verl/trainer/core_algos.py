...
def compute_policy_loss(
    ...
    ratio = torch.exp(negative_approx_kl) # 当前策略相对旧策略的变化比率
    clipped_ratio = torch.exp(torch.clamp(negative_approx_kl, np.log(1.0 - cliprange), np.log(1.0 + cliprange))) # 将 ratio 限制在 [1 - cliprange, 1 + cliprange] 之间，避免梯度爆炸
    ppo_kl = VF.masked_mean(-negative_approx_kl, eos_mask) # ppo kl损失

    pg_losses = -advantages * ratio
    pg_losses2 = -advantages * clipped_ratio # 裁剪后的policy loss
