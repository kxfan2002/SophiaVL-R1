...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        """
        # input_ids1 = micro_batch['input_ids']
        # p1 = torch.sum(input_ids1 == 151655).item()
        # print(f"forward_micro_batch input_ids1: &&&&&&&{p1}")
        ...
        input_ids = replace_discrete_image_pad(input_ids)
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            if self.config.padding_free:
                ...
                # p2 = torch.sum(input_ids_rmpad == 151655).item()
                # print(f"input_ids_rmpad: &&&&&&&{p2}")
